...
\newcommand{\kvikio}{\texttt{kvikio}\xspace}
...
scans using the fused kernels (Design and Implementation). A complete listing of all
...
tuning. 
A crucial aspect of our chunking strategy is to leverage
\kvikio \citep{kvikio2021} for seemless transfer from the \vcz store
directly to GPU memory, which mitigates much of the overhead 
due to host-to-device data transfer. %NSP 21-May-26: also decompression, maybe? Andy weigh in here please
The potential of \kvikio's GPU-native \zarr backend for accelerating 
the processing of population genetic data was pointed out by
\citet{czech2024vcz}, and has proven essential for the efficiency of the
streaming-to-GPU strategy implemented in \pggpu.
